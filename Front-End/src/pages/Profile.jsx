...
import { User, Mail, Phone, MapPin, Shield, Wallet, IndianRupee } from "lucide-react";
...
export const Profile = () => {
  ...
  const fetchWalletAmount = async () => {
    try {
      const response = await axiosInstance.post(`${baseURL}/trips/completed/byCarrier`, {
        email: storedEmail,
      });
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res = await axiosInstance.post(`${baseURL}/api/payout/send`, {
        name: profile.name,
        email: profile.email,
        upi: upiId,
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10">
            <div className="bg-white shadow-lg rounded-xl p-6">
              ...
              {withdrawStatus && (
                  <p className="text-sm mt-3 text-center text-blue-600">{withdrawStatus}</p>
              )}
            </div>
          </div>
        </div>
